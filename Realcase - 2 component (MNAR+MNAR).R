...
#E-step
test_t <- test_1
for (k in 501:501) { #iteration for EM
    ...
    if (test_1$R[i]==0){ #R=0 means missing
      ...
      for (index in 1:1000) {
        ...
        if (gibbs[index+1, 3] == 1){
          ...
          # current EM iteration parameter value for cluster 2 (still MNAR)
          b0_e <- para[k,49]
          b1_e <- para[k,50]
          b2_e <- para[k,51]
          b3_e <- para[k,52]
  ...
  fit_R2 <- glm(formula=R~income+age+gender+educ+race+disease+insu+exp_r, d2, family = binomial(link = "probit"), weights=w2)
  para[k+1,57:65] <- fit_R1[["coefficients"]]
  para[k+1,66:74] <- fit_R2[["coefficients"]]
...
saveRDS(test_1, file="/Users/riceball/Desktop/PhD/Paper/Paper 1 - Mixture MNAR/realcase_2 component_test_1.csv")
saveRDS(para, file="/Users/riceball/Desktop/PhD/Paper/Paper 1 - Mixture MNAR/realcase_2 component_para.csv")
